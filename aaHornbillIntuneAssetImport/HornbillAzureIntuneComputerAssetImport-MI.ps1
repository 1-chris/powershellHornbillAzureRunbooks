...
try {
    ...
    $ManagedDevices = Get-MgDeviceManagementManagedDevice -All | Where-Object {$_.OperatingSystem -eq 'Windows' -or $_.OperatingSystem -eq 'MacOS'}
    Write-Output "Managed device count: $($ManagedDevices.Count)"

    foreach ($Device in $ManagedDevices) {

        $AssetType = "1" # 1: Desktop, 2: Laptop
        # Blunt logic, if device has WiFi mac address, mark as laptop. Graph Beta managedDevice objects have chassisType property which can be desktop/laptop etc. 
        if ($null -ne $Device.WiFiMacAddress -and $Device.WiFiMacAddress -ne "") {
            $AssetType = "2"
        }
        ...
        if( $null -ne $AssetIDCheck.AssetID) {
            ...
            Add-HB-Param        "h_description" $Device.ManagedDeviceName
            Add-HB-Param        "h_mac_address" $Device.EthernetMacAddress
            Add-HB-Param        "h_wmac_address" $Device.WiFiMacAddress
